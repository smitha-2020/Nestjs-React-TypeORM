...
function CreatePhoto() {
  ...
  const [returnedData, setReturnedData]: [File, Function] = useState<any>();

  const changeUrl = (file: any) => {
  ...
  useEffect(() => {
    ...
    postImage(fileData)
      .then((returnedData) => setReturnedData(returnedData))
      .catch(console.error);
  }, [fileData]);
  ...
  const onSubmitHandler = (data: CreatePhotoInfo) => {
    const newData: CreatePhotoInfo = {
      ...data,
      author: data.author,
      width: String(data.width),
      height: String(data.height),
      copiesDownloaded: String(data.copiesDownloaded),
      downloadUrl: `http://localhost:3000/photos/${returnedData}`,
    };

    postPhoto(newData).then().catch(console.error);
    reset({
      author: "",
      width: "",
      height: "",
